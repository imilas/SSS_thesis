...
\textbf{FC-DVN}: Fully connected network trained on Envelope features, reaching 97\% accuracy on our test data for~\decfirst. With size of 10x5x10. \\
\begin{center}
\vbox{
...
As discussed in Section~\ref{surveys}, higher accuracy rates in these models do not translate to higher agreeableness with humans. Model accuracy on test data alone cannot be relied upon when the domain of sounds that are categorized is switched from original percussion samples to virtual synth sounds.
