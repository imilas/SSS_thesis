...
% Agreeability scores for MEM pipeline are often lower, but we reiterate that this can be caused by having two out of six (rather than one out of six in the TPE pipeline) categories which are exclusive to survey responders. Supporting this hypothesis is the notable improvement in agree-ability observed after dropping all \enquote{Bad} and \enquote{other} samples. Here, both persons agree with each other and with the MEM at a stronger rate than in the TPE pipeline, despite having similar number of samples (120 for MEM and 154 for TPE) and equal number of categories to choose from. This values are observed in the last rows of tables~\ref{kappa_table_TPE} and~\ref{kappa_table_MEM}. The higher DVD categorization accuracy achieved by the MEM compared to the best TPE is consistent with this observation: Given that the sound being categorized is a drum, the MEM will yield better results that the TPE. However, the MEM pipeline does not yield better results in the DVN task. 
