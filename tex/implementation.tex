...
\end{table}
To better understand the delineation potential of encoder features from the most successful networks, we trained a CNN network using \%50 of db1. The remaining \%50 as well as approximately 1000 randomly selected sounds from NoiseDB were encoded into 8 dimensions. These 8-dimensions were then projected onto a 2 or 3-dimensional plane using t-SNE. Referring to~\hypref{Decision.1} and~\hypref{Decision.2} to define our expectation of a useful encoder, we expect the drum sounds to be clustered together, with the majority of the synthetic noise sounds appearing away from these clusters. Our projections confirm that this is the case. We are most interested in synthetic noise sounds which appear within or near these clusters. Do these synthetic noise sounds have drum like characteristics?\\ 
\begin{figure}[htbp!]
\centering
...
As t-SNE results are not classifiers in of themselves, we create a model which uses encoded versions of each drum sound to predict its type. To address the problems mentioned in~\ref{fig:embedding_FE}, we include the envelope features along with the embeddings. 
