...
We reuse the model trained on  MixedDB as our embedding feature extractor. We use a combination of RadarDB and FreeDB and NoiseDB for training. We only focus on clap,hat,kick,snares, synthetic noise groups for measuring effectiveness to prevent class overlaps as much as possible. We also exclude samples longer than 1 second, to exclude potentially mislabeled data. Our final training database for mixed ear models is described in table~\ref{db:memDB}.

\subsubsection{Model Selection}
Using our encoding and envelope features to represent audio, five classification algorithms were trained for the task of categorizing the five different sound groups.
The specifications of these algorithms are described in table~\ref{table:mem_model_selection}. The models were trained using 5-fold cross validation and 80/20 train-to-test ratio. ~\ref{fig:mem_model_selection}. 
...
\end{tabular}
\caption{Database 3: A database put together by combination of radarDB, FreeDB and NoiseDB}
\label{db:memDB}
\end{table}
