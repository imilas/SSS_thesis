...
Using our encoding and envelope features to represent audio, five classification models were trained for the task of categorizing the five different sound groups. For both hyper-parameter optimization, the models were trained using 5-fold cross validation and 80/20 train-to-test ratio, with the average F-Score being used as the performance measurement. For inter-model comparisons, the procedure is the same except 10-fold cross validation is used. Our models were derived from scikit-learn's implementations of these classifiers~\cite{pedregosa2011scikit}. Before inter-model comparisons, we conducted a grid-search for each model on at least one of its possibly decisive hyper-parameters. The classifiers and other notable specifications are presented in table~\ref{table:mem_model_selection}. We introduced class weights where possible to mitigate the effects of an imbalanced dataset~\cite{provost2000machine,chawla2004special}. When used, class weights are calculated as the ratio of the number of samples in each class to the number of total samples. 
...
Model name & Tuned Parameters\tnote{\dag}  & Used Weights? \tnote{\ddag} \\\hline
 ...
 LinearSVC & C:10 & Yes\\
 K Nearest Neighbours & Num. Neighbours:30 &  No \\
 RandomForestClassifier & Num Estimators:500 - & Yes \\
...
\subsubsection{Addition of Envelope Features to Embedding}
