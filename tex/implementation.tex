...
The second question can be answered rather easily and in various ways, as later discussed. However, the former decision requires knowledge of what drums \textbf{do not} sound like, or knowledge of an infinitely large set, which cannot be fully represented via examples. It also means that the source of sounds used in training the model (organic drum sounds) will be fundamentally different from the source of unlabeled sounds we wish to categorize (noise from a synthesizer). We attribute the relative ease of the latter decision compared the former to the OSR problem. 
