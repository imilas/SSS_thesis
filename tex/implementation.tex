...
\textbf{2 Dimensional Projection of Latent Variables}\par\medskip
 \includegraphics[width=0.90\linewidth]{images/chapter3/t-SNE_2d.pdf}
...
As t-SNE results are not classifiers in of themselves, we create a model which uses encoded versions of each drum sound to predict its type. To address the problems mentioned in~\ref{fig:embedding_FE}, we include the envelope features along with the embedding and measure the change in precision, recall and accuracy before and after the inclusion of envelope features.

We reuse the model trained on \%80 of MixedDB as feature extractor. We use a combination of RadarDB and FreeDB and NoiseDB for training. We only focus on clap,hat,kick,snares, synthetic noise groups for measuring effectiveness to prevent class overlaps as much as possible. Our final training database for mixed ear models is described in table~\ref{table:memDB}.
